...
def enhancedFeatureExtractorDigit(datum):
    ...
    breaks, index, above, right = 0, 0, 0, 0
    ...
    for i in range(len(pixels)):
        for j in range(1,len(pixels[i])):
            if pixels[i][j] != 0:
                index += 1
                if not firstLeft or j < firstLeft: firstLeft = j
                if j <= (len(pixels) + 1) / 2: above += 1
    ...
    for n in range(5):
        ...
        features[-n] = index > 300 and 1.0 or 0.0

    for n in range(10):
        features[(n + 1) * 10] = aspectRatio < 0.69

    percentAbove = float(above) / index
    for n in range(5):
        features[-(n + 1) * 10] = percentAbove > 0.35 and 1.0 or 0.0

    percentRight = float(right) / index
