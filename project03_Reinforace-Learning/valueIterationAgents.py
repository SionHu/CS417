...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for _ in range(iterations):
            newV = self.values.copy()  # copy the values
            for state in self.mdp.getStates():
                if self.mdp.isTerminal(state): continue
                else:
                    actions = self.mdp.getPossibleActions(state) # get all possible actions
                    arr = []
                    for a in actions: arr.append(self.getQValue(state, a))
                    newV[state] = max(arr) # update the new values with the best result
            self.values = newV # update the values to the new values
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, probability in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            temp = reward + (self.discount * self.values[nextState])
            qValue.append(probability * temp)
        result = sum(qValue)
        return result
